...
namespace Unity.Entities.VR.Rendering
{
    ...
    // Copied from MeshInstanceRendererSystem
    // Doesn't draw element that doesn't have Tracked component on them
    ...
    /// </summary>
    [ExecuteInEditMode]
    public class TrackedMeshInstanceRendererSystem : ComponentSystem
    {
        ...
        protected override void OnCreateManager()
        {
            m_FrozenChunksQuery = GetComponentGroup(new EntityArchetypeQuery
            {
                ...
                All = new ComponentType[] { typeof(LocalToWorld), typeof(TrackedMeshInstanceRenderer), typeof(VisibleLocalToWorld), typeof(Frozen) }
            });
            m_DynamicChunksQuery = GetComponentGroup(new EntityArchetypeQuery
            {
                Any = Array.Empty<ComponentType>(),
                None = new ComponentType[] { typeof(Frozen)  },
                All = new ComponentType[] { typeof(LocalToWorld), typeof(TrackedMeshInstanceRenderer), typeof(VisibleLocalToWorld),typeof(Tracked) }
            });
            ...
            GetComponentGroup(new EntityArchetypeQuery
            {
                ...
                All = new ComponentType[] { typeof(TrackedMeshInstanceRenderer), typeof(LocalToWorld) , typeof(Tracked) }
            });
        ...
        struct UpdateChunkBounds : IJobParallelFor
        {
            ...
            [ReadOnly]
            public ArchetypeChunkComponentType<WorldMeshRenderBounds> WorldMeshRenderBoundsType;
        ...
        unsafe struct CullLODToVisible : IJobParallelFor
        {
            ...
            [ReadOnly]
            public WorldMeshRenderBounds* ChunkBounds;
            [ReadOnly]
        ...
        struct MapChunkRenderers : IJobParallelFor
        {
            ...
            public NativeArray<ArchetypeChunk> Chunks;
            [ReadOnly]
            public ArchetypeChunkSharedComponentType<TrackedMeshInstanceRenderer> MeshInstanceRendererType;
        ...
        unsafe void UpdateFrozenInstanceRenderer()
        {
            ...
            var meshInstanceRendererType = GetArchetypeChunkSharedComponentType<TrackedMeshInstanceRenderer>();
        ...
        {
            var renderer = EntityManager.GetSharedComponentData<TrackedMeshInstanceRenderer>(lastRendererIndex);
